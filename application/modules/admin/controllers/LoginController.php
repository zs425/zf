...
class Admin_LoginController extends Zend_Controller_Action
{
    ...
    public function loginAction()
    {
        ...
        $dbAdapter = new Zend_Auth_Adapter_DbTable($db, 'user', 'email', 'password', 'SHA1(?)');

        // création des validateurs
        $validator = new Zend_Validate();
        $validator->addValidator(new Zend_Validate_NotEmpty());
        $validator->addValidator(new Zend_Validate_StringLength(3));
    ...
    private function _setAcls(stdClass $user)
    {
        ...
        $reservations = $TReservation->fetchAll($TReservation->select()->from($TReservation, 'id'))->toArray();
        ...
        if ($user->is_admin == 1) {
            ...
            $acl->allow('user', 'reservations', 'ajouter', new Zfbook_Acl_AddReservationAssertion($user->id, $this->getInvokeArg('config')->maxreservations));
        }*/
    }
